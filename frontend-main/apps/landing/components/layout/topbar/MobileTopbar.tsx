...
import { socialLink, topbarMobileLinks } from "@repo/ui/constants/constant";
          ...
          <Flex flexDirection={"column"} alignItems={"flex-start"} gap={"8px"}>
            ...
            >
              ...
              <Text
                ...
                color={"primary.600.light"}
                onClick={() => {
                  router.push(`${directoryUrl}/client-sign-up`);
                }}
              >
                Sign up
              </Text>
    ...
    >
      ...
      >
        ...
        <List listStyleType={"unset"} padding={"16px 32px"}>
          {topbarMobileLinks.map((item, index) => {
            if (item.subMenuLinks) {
              return (
                <ListItem
                  key={item.key}
                  mb={index === topbarMobileLinks.length - 1 ? 0 : "32px"}
                >
                  <Button
                    type="button"
                    bg={"transparent"}
