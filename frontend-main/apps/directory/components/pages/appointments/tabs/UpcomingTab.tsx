...
import SendReminderModal from "../modals/upcoming/SendReminderModal";
import ViewDetailsModal from "../modals/upcoming/ViewDetailsModal";
import CancelAppointmentModal from "../modals/upcoming/CancelAppointmentModal";
import {
...
const UpcomingTab = () => {
  ...
  const { debouncedSearchValue, setUpcomingDataCount } =
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: (options) => {
      return getAppointmentList({
        ...
        status: options?.queryKey[4] as "Confirmed",
      });
    },
  });
  ...
  useEffect(() => {
    if (data?.data?.meta?.totalDocs) {
      setUpcomingDataCount(data.data.meta.totalDocs);
    }
  }, [data]);
