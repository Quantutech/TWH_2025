...
import { forgotPasswordProvider } from "@repo/ui/utils/api";
import { useState } from "react";
import { FormProvider, useForm } from "react-hook-form";
import { validationSchemaForgotPassword } from "../../../validations/clientSignUpValidations";
const ProviderForgotPassword = () => {
  ...
  const onSubmit = async (data: Record<string, any>) => {
    try {
      setIsLoading(true);
      const response = await forgotPasswordProvider({ email: data?.email });
      if (response?.data) {
        setIsLoading(false);
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              >
                ...
                >
                  <Button
                    type="submit"
                    bg={isLoading ? "base.white.light" : "primary.500.light"}
                    color={"base.white.light"}
                    _hover={{
                      bg: isLoading ? "base.white.light" : "primary.600.light",
                    }}
                    _active={{
                      bg: isLoading ? "base.white.light" : "primary.600.light",
                    }}
