...
import {
  clientConfirmEmailChange,
  clientRequestEmailChange,
...
const EmailEditModal = ({ data, activeModal, setActiveModal }: Props) => {
  ...
  const onSubmit = async (data: any) => {
    if (step === 0) {
      await clientRequestEmailChange();
      setStep(1);
      return;
    }
    try {
      setIsSubmitLoading(true);
      await clientConfirmEmailChange({
        newEmail: data.email,
