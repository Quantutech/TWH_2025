...
import { blogItems2 } from "@repo/ui/constants/constant"
...
const ResourcesTab = () => {
  ...
  const handleViewMore = () => {
    if (visibleItems < blogItems2.length) {
      setVisibleItems(blogItems2.length)
    } else {
      setVisibleItems(3)
    }
  }
    ...
    >
      ...
      <Flex flexWrap={"wrap"}>
        <Grid
          ...
          gap={"16px"}
        >
          {blogItems2.slice(0, visibleItems).map((item) => (
            <GridItem
              key={item.id}
              borderRadius="16px"
              overflow="hidden"
      ...
      {blogItems2?.length > 3 && (
        ...
        >
          <Text fontSize="14px" fontWeight={600} color="primary.500.light">
            {visibleItems < blogItems2.length ? "View More" : "View Less"}
          </Text>
          {visibleItems < blogItems2.length ? (
