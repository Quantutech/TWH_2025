"use client";
import React, { useMemo } from "react";
import {
  Box,
  Flex,
  Spinner,
  Table,
  Tbody,
  Td,
  Text,
  Th,
  Thead,
  Tr,
} from "@chakra-ui/react";
import { useQuery } from "@tanstack/react-query";
import { getProviderListForAdmin } from "@repo/ui/utils/api";
import { getFormattedDateTime } from "@repo/ui/utils/helpers";

const ProviderListContent = () => {
  // const [currentPage, setCurrentPage] = useState(1);
  // const [activeModal, setActiveModal] = useState<string | undefined>();
  const { data, isError, isLoading } = useQuery({
    queryKey: ["insurances", 1, 20],
    queryFn: (options) => {
      return getProviderListForAdmin(
        "",
        options?.queryKey?.[1] as number,
        options?.queryKey?.[2] as number
      );
    },
  });

  const dataSide = useMemo(() => {
    if (isError) {
      return (
        <Flex
          width={"100%"}
          justifyContent={"center"}
          alignItems={"center"}
          padding={"16px 0px"}
          height={"calc(100dvh - 206px)"}
        >
          No data found
        </Flex>
      );
    } else if (isLoading) {
      return (
        <Flex
          width={"100%"}
          justifyContent={"center"}
          alignItems={"center"}
          padding={"16px 0px"}
          height={"calc(100dvh - 206px)"}
        >
          <Spinner color="secondary.500.light" width={"40px"} height={"40px"} />
        </Flex>
      );
    } else if (data?.data?.length) {
      return (
        <Flex
          width={"100%"}
          flexDirection={"column"}
          justifyContent={"space-between"}
          padding={"16px 0px 0px 0px"}
          height={"calc(100dvh - 196px)"}
        >
          <Box overflow={"auto"} pb={"16px"}>
            <Table variant="simple">
              <Thead>
                <Tr>
                  <Th>First Name</Th>
                  <Th>Last Name</Th>
                  <Th>Email</Th>
                  <Th>Gender</Th>
                  <Th>Created At</Th>
                </Tr>
              </Thead>
              <Tbody>
                {data?.data?.map((item, index) => (
                  <Tr key={index}>
                    <Td>{item.firstName || "-"}</Td>
                    <Td>{item.lastName || "-"}</Td>
                    <Td>{item.email || "-"}</Td>
                    <Td>{item.gender || "-"}</Td>
                    <Td>
                      {item.createdAt
                        ? getFormattedDateTime(item.createdAt.toString())
                        : "-"}
                    </Td>
                  </Tr>
                ))}
              </Tbody>
            </Table>
          </Box>
          {/* <Pagination
            currentPage={currentPage}
            totalPages={data?.data?.meta?.totalPages || 0}
            onPageChange={(page) => {
              setCurrentPage(page);
            }}
          /> */}
        </Flex>
      );
    }
  }, [data, isLoading, isError]);

  return (
    <Box
      padding={"16px 24px"}
      border={"1px solid"}
      borderColor={"secondary.100.light"}
      borderRadius={"16px"}
      mt={4}
      backgroundColor={"base.white.light"}
    >
      <Text
        mr={"auto"}
        fontWeight={600}
        fontSize={"20px"}
        color={"secondary.950.light"}
      >
        Provider List
      </Text>
      {dataSide}
    </Box>
  );
};

export default ProviderListContent;
