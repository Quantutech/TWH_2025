...
import { addSpecialities } from "@repo/ui/utils/api";
...
const CreateModal = ({ activeModal, setActiveModal, refetch }: Props) => {
  ...
  const onSubmit = async (formData: { name: string }) => {
    try {
      await addSpecialities({
        name: formData?.name,
      });
      await refetch();
      reset();
      showToast("Success", "Specialities create successfully.", "success");
      setActiveModal(undefined);
      setIsSubmitLoading(false);
    } catch (error) {
      setIsSubmitLoading(false);
      showToast("Error", "Failed to create Specialities.", "error");
    }
  };
